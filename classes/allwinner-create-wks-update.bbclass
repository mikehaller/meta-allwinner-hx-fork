...
#### Partitions specific to sunxi64 images
        ...
        cat >> "$wks" <<EOF
part SPL --source rawcopy --sourceparams="file=${SPL_NAME}" --ondisk ${SUNXI_STORAGE_DEVICE} --no-table --align 8
part u-boot --source rawcopy --sourceparams="file=${UBOOT_BINARY}" --ondisk ${SUNXI_STORAGE_DEVICE} --no-table --align 40
EOF
    fi

#### Add boot partition and separate rootfs partition if SUNXI_BOOT_IMAGE is set in conf/machine/include/allwinner-defs.inc
...
part --source rootfs --ondisk ${SUNXI_STORAGE_DEVICE} --fstype=ext4 --label root2 --align 1024 --extra-space ${ROOT_EXTRA_SPACE}
EOF
else
    cat >> "$wks" <<EOF
part / --source rootfs --ondisk ${SUNXI_STORAGE_DEVICE} --fstype=ext4 --label root --align 1024 --extra-space ${ROOT_EXTRA_SPACE} --active
part --source rootfs --ondisk ${SUNXI_STORAGE_DEVICE} --fstype=ext4 --label root2 --align 1024 --extra-space ${ROOT_EXTRA_SPACE} --active
EOF
fi
#### Add swap file if SUNXI_SWAP_SIZE is set in conf/machine/include/allwinner-defs.inc
